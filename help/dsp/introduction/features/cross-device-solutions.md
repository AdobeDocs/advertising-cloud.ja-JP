...
Advertising Cloud DSPとの統合 [!DNL LiveRamp] および [!DNL Adobe Device Co-op] を使用すると、ブランドが追跡したデバイスだけでなく、ユーザーのすべての既知のデバイスに対してオーディエンスを拡張できます。 また、統合では、すべてのデバイスで頻度キャップとアトリビューション測定をおこなうこともできます。
...
* [!DNL Adobe Device Co-op]:
   * 参加しているAdobe広告主の決定論的データと確率的データのオプトインプールを提供します。
...
クロスデバイスのアトリビューション測定を有効にする場合は、 [!DNL Adobe] アカウントチーム。 の場合 [!DNL Adobe Device Co-op] アカウントに署名済みの [!DNL Adobe Device Co-op] 契約とExperience Cloud組織 ID( 以前の [!DNL IMS org ID]) をクリックします。
...
1. 内 [!UICONTROL Integrations] 広告主設定のセクションで、 [!UICONTROL Cross-Device Attribution] の設定がアクティブです。
