...
* ネットワーク (RON) ターゲティングの実行を使用：オーディエンスのターゲティングをおこなわず、広範な在庫ターゲティングをおこなう一連のネットワーク配置を含めることが重要です。 これにより、 [!DNL Adobe Sensei] アルゴリズムを使用して、まだオーディエンスに分類されていない新しい Cookie を持っている可能性のある有用なユーザーを見つけることができます。
